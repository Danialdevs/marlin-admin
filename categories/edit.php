...
$category = $pdo->query("SELECT * FROM categories where id = '$_GET[id]'")->fetch();
if(!$category){
    header("Location: ./list.php");
    exit();
}
...
<div class="max-w-2xl mx-auto">
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        <a href="list.php" class="mb-5 font-medium text-sm inline-flex items-center text-blue-500 hover:text-blue-800">
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    if(empty($_POST["name"])){
        $_SESSION["error"] = "Требуется имя";
        header("Location: create.php");
        exit();
    }

    $pdo->query("UPDATE categories SET name = '$_POST[name]' WHERE id = '$_GET[id]'");
